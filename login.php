...
if (isset($_POST['check'])) //verifier que les checkbox sont bien check
 {  
  ...
  if ($choix == 'utilisateur') {
    ...
    $query = "SELECT * FROM utilisateur WHERE email_u='$email_u'";
    $resultat = mysqli_query($lien, $query);

    if (mysqli_num_rows($resultat) == 1) {
        $utilisateur = mysqli_fetch_assoc($resultat);
        $hashed_password = $utilisateur['mot_passe_u'];

        if (password_verify($password_u, $hashed_password)) {
            // Mot de passe correct, effectuer la redirection
            session_start();
 ...
 //fournisseur
	...
	$password_f = $_POST['pass'];
		...
		$query2 = "SELECT * FROM fournisseur WHERE email_f='$email_f'";
	    $resultat2 = mysqli_query($lien, $query2);
		//true
		   if (mysqli_num_rows($resultat2) ==1) {
			    // redirection  
				$fournisseur = mysqli_fetch_assoc($resultat2);
				$hashed_password2 = $fournisseur['mot_passe_f'];
		
				if (password_verify($password_f, $hashed_password2)) {
...
if ($choix == 'utilisateur') {
  ...
  // Vérifier si l'e-mail existe déjà dans la base de données
  $sql_check = "SELECT * FROM fournisseur WHERE email_f = '$email_f'";
  $result_check = mysqli_query($lien, $sql_check);
  if (mysqli_num_rows($result_check) ==1) {
	  // L'e-mail existe déjà, afficher un message d'erreur et arrêter l'exécution
	  echo "L'e-mail existe déjà. Veuillez en choisir un autre.<br>";
...
if (isset($_POST['user'])) {
	...
	// insert into BDD
	$sql = "INSERT INTO utilisateur (nom_u, prenom_u, email_u, mot_passe_u, date_naissance_u, poids_u, taille_u) VALUES ('$nom_u', '$prenom_u', '$email_u', '$pwd_hash','$dn_u', '$poids_u', '$taille_u')";
...
$ph=$_POST['ph'];
    ...
    $sql1="SELECT email_f FROM fournisseur WHERE IDF='$sessionid'";
